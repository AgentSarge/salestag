...
// Raw audio sample structure
typedef struct {
    uint16_t mic1_sample;  // Raw ADC value from GPIO 9 (MIC1)
    uint16_t mic2_sample;  // Raw ADC value from GPIO 12 (MIC2)
...
// Add a raw audio sample to the current recording
esp_err_t raw_audio_storage_add_sample(uint16_t mic1_adc, uint16_t mic2_adc);
